...
export default function Controls() {
    ...
    const [rewindIntervalId, setRewindIntervalId] = useState(0);
    const [forwardIntervalId, setForwardIntervalId] = useState(0);
    const [isSliderVisible, setIsSliderVisible] = useState(false);
    const [volume, setVolume] = useState(0.5); 
  

    const [mp3] = useState(new Howl({
    ...
    const forward = () => {
        let newSeek = mp3.seek() + 10; 
        newSeek = Math.min(mp3.duration(), newSeek); 
        mp3.seek(newSeek); 
        setSeek(newSeek); 
    };
