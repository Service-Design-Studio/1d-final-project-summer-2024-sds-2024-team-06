...
function SetActiveLink({href, children, ...props}){
    ...
    if (path.includes(href) || path.includes(children?.toLowerCase())){
        // return "activated style"
        return (<a href={href} className="rounded-md px-3 py-2 text-sm font-sans-700 font-bold text-orange">{children}</a>);
    }
        // else return "inactivated style"
    return(
        <a href={href} className="rounded-md px-3 py-2 text-sm font-sans-700 text-grey hover:text-orange">{children}</a>
    )
}
